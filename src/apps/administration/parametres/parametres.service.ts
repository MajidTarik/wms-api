...
export class ParametresService {
                ...
                await this.createAttributparametre(
                    {
                        ...
                        isdefault: true,
                        reforganisation: parametre.reforganisation,
                    }
                )
                    .then(async (res) => {
    ...
    async findParametre(parametreDto: ParametresFindDto) {
        return await this.parameterRepository
            ...
            .innerJoinAndSelect(
                'parametres.company',
                'company',
                'company.refcompany = COALESCE(:refcompany, company.refcompany) and company.reforganisation = COALESCE(:reforganisation, company.reforganisation)',
                {
                    refcompany: parametreDto.refcompany, reforganisation: parametreDto.reforganisation
                },
            )
            .innerJoinAndSelect(
    ...
    async showParametre(parametreDto: ParametresShowDto) {
        return await this.parameterRepository
            ...
            .andWhere('parametres.refcompany = :refcompany and parametres.refcompany = :refcompany', {refcompany: parametreDto.refcompany, reforganisation: parametreDto.reforganisation})
            .getOne()
            .then((res) => {
                return res;
            })
    ...
    async findAttributParametre(attributparametreDto: ParametresAttributeFindDto) {
        return await this.attributparametreRepository
            ...
            .innerJoinAndSelect(
                ...
                'company',
                'company.refcompany = :refcompany and company.reforganisation = :reforganisation',
                {
                    refcompany: attributparametreDto.refcompany, reforganisation: attributparametreDto.reforganisation,
                },
            )
            .innerJoinAndSelect(
    ...
    async showAttributparametre(attributparametredto: ParametresAttributeShowDto) {
        return await this.attributparametreRepository
            ...
            .andWhere(
                'parametre.refcompany = :refcompany and parametre.reforganisation = :reforganisation',
                {refcompany: attributparametredto.refcompany, reforganisation: attributparametredto.reforganisation}
            )
            .andWhere(
     ...
     */
    async checkaxesbycompany(reforganisation: string, parametreListe: object, refcompany: string, reftypeparametre: string) {
            ...
            .then(async (res) => {
                ...
                for (const key of parametreListeKey) {
                    if (!axesfromdbbycompanyMapped.includes(key)) {
                        throw new InternalServerErrorException('Vous avez envoyer un paramétres non paramétrer, veuilliez contacter votre administrateur', {
                            cause: {message: 'Vous avez envoyer un paramétre non paramétrer, veuilliez contacter votre administrateur'},
                            description: 'Vous avez envoyer un paramétres non paramétrer, veuilliez contacter votre administrateur',
                        });
                    }
                }
                for (const value of axesfromdbbycompanyMapped) {
                    if (!parametreListeKey.includes(value)) {
                        throw new InternalServerErrorException('les paramétres sont changés, veuilliez relancer à nouveau', {
                            cause: {message: 'les paramétres sont changés, veuilliez relancer à nouveau'},
                            description: 'les paramétres son changés, veuilliez relancer à nouveau',
                        });
                ...
                let parametreheaderentity = await this.parametreheaderRepository.findOneBy({
                    refcompany: refcompany,
                    refheaderparametre: refheaderparametre,
                    reforganisation: reforganisation
                });
                if (!parametreheaderentity) {
                    parametreheaderentity = await this.parametreheaderRepository.create({
                        reforganisation: reforganisation,
                        refcompany: refcompany,
                        refheaderparametre: refheaderparametre,
                        reftypeparametre: reftypeparametre
    ...
    /** OK **/
    async getOrderParametre(reforganisation, refcompany, reftypeparametre) {
        return await this.parameterRepository
            .countBy({
                refcompany: refcompany,
                reforganisation: reforganisation,
    ...
    async getIdHeaderByParametre(headerparametrerDto: ParametresHeaderFindDto) {
        return this.checkaxesbycompany(
            headerparametrerDto.reforganisation,
